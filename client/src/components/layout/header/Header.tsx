...
const Header = () => {
  ...
  const handleOpenPostModal = (e: React.MouseEvent) => {
    e.stopPropagation();
    dispatch(setIsOpen({ isOpen: true, id: 'postModal' }));
  };
                ...
                <Link to={link.to} key={link.to} className="hover:bg-navHoverColor transition ease-in rounded-full">
                  <div className="flex p-3 w-full items-center">
                    <div className="relative">
                      ...
                      {link.notifications && (
                        <div
                          className={`rounded-full absolute top-[-4px] right-[1px] w-4 h-4 ${
                            unreadNotificationsCount > 0 ? 'bg-cyan-500' : ''
                          }  flex justify-center items-center text-white text-xs`}
                        >
